...
const UserSuggest = props => {
  ...
  const renderUsers = users.map(user => <User key={user.id} id={user.id} userName={user.userName} alt={user.displayName} image={`https://api.qarun.ir/${user.userAvatar}`} self={user.self} />);
  return (
